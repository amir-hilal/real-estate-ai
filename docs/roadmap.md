...
| **2** | ML Foundation | Train an evaluated, leak-free regression model. Serialize the full preprocessing + model pipeline. | Trained model (`ml/artifacts/model.pkl`), training statistics (`ml/artifacts/training_stats.json`), ML training notebook | Phase 1 complete; feature shortlist locked | Leakage checklist complete; test-set metrics documented; model loads from disk cleanly |
| **3** | LLM Extraction Design | Design and validate the Stage 1 prompt + Pydantic schema + validation chain. | `PropertyFeatures` schema, Stage 1 prompt (`prompts/stage1_extraction_v1.md`), test query results | Phase 2 complete; feature schema locked; LLM API key configured | 10 test queries documented; T01 passes ≥8 fields; failure modes all handled |
| **4** | Prediction Interpretation | Design and validate the Stage 2 explanation prompt using grounded training statistics. | Stage 2 prompt (`prompts/stage2_explanation_v1.md`), 5 evaluation scenarios documented | Phase 2 (stats file), Phase 3 (extraction working) | 4/5 evaluation scenarios pass checklist; no hallucinated statistics confirmed |
